...
import { handleWalkAction } from "./walk-graph.js";
...
// Tool definitions: [name, description, schema, handler]
const tools: [
	string,
	string,
	...
	[
		...
		({ action, nodeId, linkId }) =>
			handleWalkAction(store, action, nodeId, linkId),
	],
	[
		"delete_link",
...
for (const [name, description, schema, fn] of tools) {
